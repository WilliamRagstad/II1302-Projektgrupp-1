...
export function testEndpoints() {
	...
	Deno.test({
		...
		async fn() {
			const getRequest = await fetch("https://airdash.herokuapp.com/data", {
				method: "GET",
				headers: {
					"Content-Type": "application/json",
				}
			});
			assertEquals(getRequest.status, 200);
		},
		sanitizeResources: false,
		sanitizeOps: false,
	});
	...
	Deno.test({
		...
		async fn() {
			const getRequest = await fetch("https://airdash.herokuapp.com/data/2021-05-01/2021-05-05", {
				method: "GET",
				headers: {
					"Content-Type": "application/json",
				}
	...
	Deno.test({
		...
		async fn() {
			const getRequest = await fetch("https://airdash.herokuapp.com/geo?query=vasastan", {
				method: "GET",
			});
			assertEquals(getRequest.status, 200);
		},
	...
	Deno.test({
		...
		async fn() {
			const getRequest = await fetch("https://airdash.herokuapp.com/video/mac-1", {
				method: "GET",
			});
			assertEquals(getRequest.status, 200);
		},
